...
class MusicPlay: SKScene,XOpenALPlayerDelegate,XMidiPlayerDelegate{
    ...
    func initLayout(){
        ...
        backgroundLayout.position = CGPoint.zero
        backgroundLayout.anchorPoint = CGPoint.zero
        backgroundLayout.zPosition = 1
        self.addChild(backgroundLayout)

        uiLayout = SKSpriteNode()
        uiLayout.size = self.size
        uiLayout.position = CGPoint.zero
        uiLayout.anchorPoint = CGPoint.zero
    ...
    func initUI(){
        ...
        buttonsLayout.anchorPoint = CGPoint.zero
        ...
        for i in 0..<midiFiles.count{
            ...
            button.initializer(postion: pos, size: s)
            button.title = midiFiles[i].fileName
            button.xClick = {
                self.buttonClick(index: i)
            }
        ...
        playControlLayout.anchorPoint = CGPoint.zero
        playControlLayout.zPosition = 1
        uiLayout.addChild(playControlLayout)
        
        progressBar = XProgressBar()
        progressBar.initializer(postion: CGPoint(x: playControlHeight, y: 0),
        ...
        playButton.initializer(postion: CGPoint.zero,
        ...
        effectLayout.anchorPoint = CGPoint.zero
    ...
    func buttonClick(index:Int){
        ...
        for n in 0..<buttons.count{
            ...
            btn.titleColor = i == index ? UIColor(red: 193/255, green: 209/255, blue: 105/255, alpha: 1) : UIColor.white
            i += 1
        }
        self.playMidi(index: index)
    }
    ...
    func playMidi(index:Int){
        ...
        let filePath = Bundle.main.path(forResource: midi.fileName, ofType: "mid")
        
        //播放MIDI URL
        var url = NSURL(fileURLWithPath: filePath!)
        midiPlayer.initMidi(url as URL!)
...
//        midiPlayer.initMidiWithData(data)
    ...
    func playingSoundNote(xMidiNoteMessage: XMidiNoteMessage!) {
        initBgShader(soundNote: Float32(xMidiNoteMessage.note))
    }
